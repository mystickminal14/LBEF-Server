...
export const createSession = asyncHandler(async (req, res) => {
  ...
  const parent = await prismaClient.academicPlanner.create({
    data: {
      type: "PARENT",
...
export const createSessionFile = asyncHandler(async (req, res) => {
  ...
  if (!parsed.success)
    throw new ApiError(400, "Validation failed", parsed.error.issues);
  const { parentId, intake, semester, course } = parsed.data;
  const parent = await prismaClient.academicPlanner.findUnique({
    where: { id: parentId },
  });
  ...
  const data = {
    ...
    course: course,
    intake: intake,
    file: `/public/planner/${file}`,
  };
  await prismaClient.academicPlanner.create({ data });
...
export const addMultipleFiles = asyncHandler(async (req, res) => {
  ...
  const data = records.map((record, index) => ({
    ...
    file: `/public/planner/${files[index].filename}`,
    session: parent.session,
  }));

  await prismaClient.academicPlanner.createMany({ data });

  res.status(201).json(new ApiResponse(201, null, "Files uploaded successfully"));
});
export const getChildrenByParentId = asyncHandler(async (req: Request, res: Response) => {
 const id = Number(req.params.id);

   const children = await prismaClient.academicPlanner.findMany({
    where: { parentId:id }, 
  });
...
export const deleteChild = asyncHandler(async (req, res) => {
  ...
  const child = await prismaClient.academicPlanner.findUnique({
    where: { id },
  });
...
export const deleteParentWithChildren = asyncHandler(async (req, res) => {
  ...
  await prismaClient.$transaction(async (tx) => {
    await tx.academicPlanner.deleteMany({
      where: { parentId },
    });

    await tx.academicPlanner.delete({
      where: { id: parentId },
    });
  });
...
export const editSession = asyncHandler(async (req, res) => {
  ...
  const parent = await prismaClient.academicPlanner.findUnique({ where: { id } });
  if (!parent || parent.type !== "PARENT")
    throw new ApiError(404, "Parent not found");

  const updated = await prismaClient.academicPlanner.update({
    where: { id },
...
export const getParents = asyncHandler(async (_req, res) => {
  const data = await prismaClient.academicPlanner.findMany({
    where: { type: "PARENT" },
    orderBy: { year: "desc" },
  });
...
export const getParentWithChildren = asyncHandler(async (req, res) => {
  const id = Number(req.params.id);
   ...
   const children = await prismaClient.academicPlanner.findMany({
    where: { parentId:null ,},orderBy: { year: 'desc' }, include:{children:true}
  });
