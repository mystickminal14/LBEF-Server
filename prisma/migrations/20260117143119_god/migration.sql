/*
  ...
  - Made the column `semester` on table `academicplanner` required. This step will fail if there are existing NULL values in that column.
  - Made the column `intake` on table `academicplanner` required. This step will fail if there are existing NULL values in that column.

*/
-- DropIndex
DROP INDEX `AcademicPlanner_academicYearId_plannerCourseId_idx` ON `academicplanner`;

-- DropIndex
DROP INDEX `AcademicPlanner_plannerCourseId_fkey` ON `academicplanner`;

-- DropIndex
DROP INDEX `AcademicPlanner_semester_key` ON `academicplanner`;
...
-- AlterTable
ALTER TABLE `academicplanner` MODIFY `semester` VARCHAR(191) NOT NULL,
    MODIFY `intake` VARCHAR(191) NOT NULL;
