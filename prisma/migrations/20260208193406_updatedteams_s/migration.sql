/*
  ...
  - You are about to drop the column `department` on the `ourteam` table. All the data in the column will be lost.
  - Added the required column `status` to the `TeamDept` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE `ourteam` DROP COLUMN `department`,
    ADD COLUMN `departmentId` INTEGER NULL;

-- AlterTable
ALTER TABLE `teamdept` ADD COLUMN `status` ENUM('ENABLED', 'DISABLED') NOT NULL;
