...
CREATE TABLE `Permission` (
    ...
    `name` ENUM('USERS', 'COURSES', 'TEAMS', 'TEAM_DEPT', 'FAQ', 'ALUMNI', 'HERO_SECTION', 'PLANNER_COURSE', 'NEWS', 'ALMUNI_FORM', 'JOURNALS', 'EDITORIAL_BOARD', 'CONNECT', 'GALLERY', 'SCHOLARSHIP', 'NOTICE', 'CONTACT', 'HOLIDAY', 'RECOGNITION', 'ACHIEVEMENT', 'INTAKE', 'DOCUMENTS', 'ACADEMIC_PLANNER', 'FEE_PLANNER', 'DOWNLOADS') NOT NULL,
...
CREATE TABLE `User` (
    ...
    `role` ENUM('ADMIN', 'SUPERADMIN', 'MANAGER', 'USER') NOT NULL,
...
CREATE TABLE `AdmissionIntake` (
    ...
    `lastDateStatus` ENUM('ENABLED', 'DISABLED') NOT NULL,
...
-- CreateTable
CREATE TABLE `PlannerCourse` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `name` VARCHAR(191) NOT NULL,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NOT NULL,

    UNIQUE INDEX `PlannerCourse_name_key`(`name`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `GalleryType` (
    ...
    UNIQUE INDEX `GalleryType_name_key`(`name`),
    UNIQUE INDEX `GalleryType_slug_key`(`slug`),
    FULLTEXT INDEX `GalleryType_name_idx`(`name`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- CreateTable
CREATE TABLE `AlumniForm` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `collegeRollNo` VARCHAR(191) NOT NULL,
    `uniRollNo` VARCHAR(191) NOT NULL,
    `prefix` ENUM('MR', 'MS') NOT NULL,
    `fullName` VARCHAR(191) NOT NULL,
    `degree` VARCHAR(191) NOT NULL,
    `yearOfPassing` VARCHAR(191) NOT NULL,
    `mode` ENUM('MORNING', 'DAY', 'EVENING', 'WEEKEND') NOT NULL,
    `email` VARCHAR(191) NOT NULL,
    ...
    UNIQUE INDEX `AlumniForm_collegeRollNo_key`(`collegeRollNo`),
...
-- AddForeignKey
ALTER TABLE `FAQ` ADD CONSTRAINT `FAQ_parentId_fkey` FOREIGN KEY (`parentId`) REFERENCES `FAQ`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;
